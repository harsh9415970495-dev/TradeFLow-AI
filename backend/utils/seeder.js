...
const stocksData = [
  ...
  { symbol: 'M&M.NS', companyName: 'Mahindra & Mahindra', sector: 'Automobile', overview: 'Mahindra & Mahindra Limited is an Indian multinational automotive manufacturing company. It is one of the largest vehicle manufacturers by production in India.' },
  ...
  { symbol: 'WIPRO.NS', companyName: 'Wipro Limited', sector: 'Information Technology', overview: 'Wipro Limited is an Indian multinational corporation that provides information technology, consulting and business process services.' },
  { symbol: 'HCLTECH.NS', companyName: 'HCL Technologies', sector: 'Information Technology', overview: 'HCL Technologies Limited is an Indian multinational information technology company, that is headquartered in Noida.' },
];
...
async function main() {
  ...
  try {
    ...
    for (const stock of stocksData) {
      try {
        // Try query1, fallback to query2 for reliability
        let response = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${stock.symbol}?interval=1d&range=1mo`);
        if (!response.ok) {
          response = await fetch(`https://query2.finance.yahoo.com/v8/finance/chart/${stock.symbol}?interval=1d&range=1mo`);
        }
        ...
        console.log(`Successfully fetched and seeded ${stock.symbol}`);
      } catch (err) {
